...
import { getDownloadListSync } from '@/core/download'

const getTargetList = (listId: string | null): LX.Music.MusicInfo[] | LX.Download.ListItem[] => {
  return listId == LIST_IDS.DOWNLOAD ? getDownloadListSync() : getListMusicSync(listId)
}
...
export const getPlayIndex = (listId: string | null, musicInfo: LX.Download.ListItem | LX.Music.MusicInfo | null, isTempPlay: boolean): {
  ...
  const playerList = getTargetList(playInfo.playerListId)
